  ...
  async execute(client, message, args, ee) {
    try {
      ...
      if (member) {
        ...
        if (!parseInt(args[0]) || parseInt(args[0]) < 1 || parseInt(args[0]) > 1000) return message.reply({ embeds:[new MessageEmbed()
          .setColor(ee.wrongcolor)
          .setFooter(ee.footertext, ee.footericon)
          .setDescription(`The number specified must be between 1 and 1000.`)]});
